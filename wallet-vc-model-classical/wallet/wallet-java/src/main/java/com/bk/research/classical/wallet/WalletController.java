...
public class WalletController {
    ...
    @Value("${BANK_ISSUER_URL:http://tls-wallet-outbound:18443}")
    private String bankIssuerUrl;

    @Value("${KEYCLOAK_URL:http://tls-wallet-outbound:18443}")
    ...
    public String getWalletKey() {
        JSONObject res = new JSONObject();
        res.put("jwk", new JSONObject(cryptoService.getEcJwk().toPublicJWK().toJSONString()));
    ...
    public String getJwks() {
        JSONObject res = new JSONObject();
        res.put("keys", new JSONArray().put(new JSONObject(cryptoService.getEcJwk().toPublicJWK().toJSONString())));
        return res.toString();
    }
    ...
    public ResponseEntity<String> fetchOffer(@RequestParam(name = "type", defaultValue = "identity") String type) {
        try {
            ...
            logProtocol("Fetching Credential Offer", "outgoing",
                "Fetching Credential Offer from Bank Issuer via ECDSA mTLS: " + offerUrl, null);

            ResponseEntity<String> offerRes = restTemplateConfig.getRestTemplate()
    ...
    public String receiveOffer(@RequestBody String bodyStr) throws Exception {
        ...
        try {
            ...
            ResponseEntity<String> metaRes = restTemplateConfig.getRestTemplate()
    ...
    public String startAuth(@RequestBody String bodyStr) throws Exception {
        ...
        try {
            ...
            ResponseEntity<String> parRes = restTemplateConfig.getRestTemplate().postForEntity(
            ...
            System.err.println("[Wallet] PAR fallback: " + e.getMessage());
            authUrl = frontendKeycloakUrl + "/realms/fapi-demo/protocol/openid-connect/auth" +
                "?client_id=" + clientId +
                "&response_type=code" +
        ...
        logProtocol("Auth Redirect", "outgoing", "Redirecting user to Keycloak login", null);
    ...
    public RedirectView callback(@RequestParam String code, @RequestParam String state) throws Exception {
        ...
        logProtocol("Token Exchange", "outgoing", "Exchanging code for token via ECDSA mTLS", null);
        ...
        String proofJwt = cryptoService.signProof(proofPayload.toString(), "openid4vci-proof+jwt");
        ...
        logProtocol("Credential Request", "outgoing", "Requesting VC from Bank Issuer via ECDSA mTLS", null);

        ResponseEntity<String> credRes = restTemplateConfig.getRestTemplate().postForEntity(
        ...
        logProtocol("Credential Received", "incoming",
            "Classical VC stored in wallet: " + session.getString("credConfig"),
    ...
    public synchronized String ensureAuthorizationVc() {
        ...
        try {
            logProtocol("Auto-Provisioning Authorization Credential", "outgoing",
                "Requesting initial Scope VC from Bank Issuer", null);
            ...
            if (credRes.getStatusCode().is2xxSuccessful()) {
                ...
                logProtocol("Scope VC Auto-Provisioned", "incoming",
                    "Stored Bank-signed Root Authorization VC: " + jti, vc);
                return jti;
            }
        } catch (Exception e) {
    ...
    public ResponseEntity<String> fetchTppRequest(
        ...
        try {
            ...
            ResponseEntity<String> tppRes = restTemplateConfig.getRestTemplate().getForEntity(fetchUrl, String.class);
        ...
        try {
            ...
            if (targetTppUrl != null && !targetTppUrl.isEmpty()) {
                ...
                try {
                    ...
                    // Build DAS-only encrypted package (F3-03)
                    ...
                    restTemplateConfig.getRestTemplate().postForEntity(
                    ...
                    System.err.println("[Wallet] Auto-submit presentation to TPP: " + pushEx.getMessage());
    ...
    public String sendToTpp(@RequestBody String bodyStr) {
        ...
        logProtocol("Sending VCs to TPP", "outgoing", "Sending via ECDSA mTLS", null);
    ...
    private ECKey fetchBankEcKey() {
        try {
            ...
            System.err.println("[Wallet] Error fetching bank JWKS: " + e.getMessage());
        }
        return null;
    }
    ...
    private String buildDasEncryptedPackage(String tppClientId, String rDas, String authzVcJti) {
        try {
            ...
            JWEHeader header = new JWEHeader.Builder(JWEAlgorithm.ECDH_ES, EncryptionMethod.A256GCM)
                    .keyID(bankKey.getKeyID())
                    .build();
            JWEObject jweObject = new JWEObject(header, new Payload(sensitivePayload.toString()));
            jweObject.encrypt(new ECDHEncrypter(bankKey.toECPublicKey()));
            return jweObject.serialize();
        } catch (Exception e) {
            System.err.println("[Wallet] Failed to build DAS encrypted package: " + e.getMessage());
            return null;
        }
    }
    ...
    private String buildAndStoreDelegateVc(String authzVcJti, String tppClientId,
        ...
        try {
            ...
            payload.put("wallet_ec_jwk", walletEcPublicJwk);
            ...
            String delegateJwt = cryptoService.sign(payload.toString(), "vc+sd-jwt");

            // RFC 9901 Key Binding JWT (KB-JWT) with SHA-256 digest per Classical design specification
            java.security.MessageDigest md = java.security.MessageDigest.getInstance("SHA-256");
            ...
            String kbJwt = cryptoService.sign(kbPayload.toString(), "kb+jwt");
            ...
            logProtocol("Delegate VC Created", "internal",
                "Signed Delegate VC for TPP: " + tppClientId + " scopes: " + delegatedScopes + " with RFC 9901 Key Binding", null);
