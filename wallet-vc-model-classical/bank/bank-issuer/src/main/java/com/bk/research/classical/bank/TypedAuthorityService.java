...
/**
 * Fail-closed implementation of the S/A/V/D authority meet defined by the
 * evaluation contract. Inputs remain attributable to SC, H, R, T and P; this
 * service only evaluates them and never invents a missing authority source.
 */
@Service
public class TypedAuthorityService {
    ...
    public long getEvaluationTime() {
        return configuredEvaluationTime > 0 ? configuredEvaluationTime : System.currentTimeMillis() / 1000;
    }

    public Decision evaluate(JsonNode sc, JsonNode h, JsonNode r, JsonNode t) {
        JsonNode p = policyAuthority;
        if (p == null) {
            return deny("REJECT_MISSING_POLICY_AUTHORITY");
        }
        validateShape(sc, "SC");
        validateShape(h, "H");
        validateShape(r, "R");
        ...
        ObjectNode validity = effective.putObject("validity");
        validity.put("start", start);
        validity.put("end", end);
        ObjectNode data = effective.putObject("data");
        data.put("history_days", historyDays);
        putSorted(data.putArray("fields"), fields);
    ...
    private void validateShape(JsonNode authority, String source) {
        if (authority == null || !authority.isObject()
                || !isNonEmptyArray(authority.get("services"))
                || !isNonEmptyArray(authority.get("accounts"))
                || !authority.path("validity").path("start").canConvertToLong()
                || !authority.path("validity").path("end").canConvertToLong()
                || !authority.path("data").path("history_days").canConvertToLong()
                || !isNonEmptyArray(authority.path("data").get("fields"))
                || !authority.path("data").path("page_size").canConvertToLong()) {
