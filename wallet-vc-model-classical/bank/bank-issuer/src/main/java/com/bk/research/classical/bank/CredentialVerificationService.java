...
public class CredentialVerificationService {
    ...
    /**
     * Does this presentation carry its own Holder key and verify under it instead of under the
     * issuer-protected key from the issuance record? Used only to classify a rejection: a key advertised
     * inside the presentation is never accepted as a verification key.
     */
    private boolean verifiesUnderAdvertisedKey(JWSObject jwsObject, JsonNode payload, String authoritativeJwkJson) {
        ...
        for (String claim : new String[] {"wallet_ec_jwk", "holder_key_hint"}) {
            ...
            if (advertised == null || advertised.isBlank() || advertised.equals(authoritativeJwkJson)) continue;
            try {
                ECKey advertisedKey = ECKey.parse(advertised);
                if (jwsObject.verify(new ECDSAVerifier(advertisedKey.toECPublicKey()))) {
                    return true;
                }
    ...
    public VerificationResult verifyRootScopeVc(String sdJwtStr, String bankIssuerUrl) {
        try {
            ...
            // 3. Verify ECDSA signature of Issuer
            try {
                JWSObject jwsObject = JWSObject.parse(jwtPart);
                ECDSAVerifier verifier = new ECDSAVerifier(cryptoService.getEcJwk().toECPublicKey());
                if (!jwsObject.verify(verifier)) {
                    return new VerificationResult(false, "REJECT_INVALID_ROOT_VC_SIGNATURE: Root Scope VC signature verification failed", header, payload, null, scopes);
                }
            } catch (Exception sigEx) {
                return new VerificationResult(false, "REJECT_INVALID_ROOT_VC_SIGNATURE: Root Scope VC signature check error: " + sigEx.getMessage(), header, payload, null, scopes);
            }
            ...
            if (holderCertRef != null) {
                ...
                if (!payload.has("cnf") || !payload.get("cnf").has("jwk")
                    ...
                    return new VerificationResult(false, "REJECT_BINDING_KEY_SET_MISMATCH: Holder verification key differs from committed issuance state", header, payload, record, scopes);
                }
    ...
    public VerificationResult verifyDelegationVc(String delegateSdJwtStr, VerificationResult rootResult, String expectedDelegateClientId) {
        try {
            ...
            // 3b. Containment Check: delegated_scopes ⊆ rootScopes (Typed Non-Expansion with semantic alias tolerance)
            ...
            // 4. Verify Holder Signature / Chained Proof (Strict Fail-Closed)
            ...
            try {
                ECKey holderKey = ECKey.parse(certRecord.getRawJwkJson());
                JWSObject jwsObject = JWSObject.parse(jwtPart);
                ECDSAVerifier verifier = new ECDSAVerifier(holderKey.toECPublicKey());
                if (!jwsObject.verify(verifier)) {
                    ...
                    String rejection = verifiesUnderAdvertisedKey(jwsObject, payload, certRecord.getRawJwkJson())
                            ? "REJECT_SUBSTITUTED_HOLDER_KEY"
                            : "REJECT_INVALID_DELEGATION_VC_SIGNATURE";
                    return new VerificationResult(false, rejection + ": Delegation VC signature verification under authoritative Holder key failed", header, payload, certRecord, delegatedScopes);
                }
            } catch (Exception ex) {
                return new VerificationResult(false, "REJECT_INVALID_DELEGATION_VC_SIGNATURE: Error verifying Holder signature on Delegation VC: " + ex.getMessage(), header, payload, certRecord, delegatedScopes);
            }
            ...
            if (parts.length > 1 && !parts[parts.length - 1].isEmpty()) {
                ...
                if (kbSections.length >= 2) {
                    try {
                        ...
                        // Validate sd_hash: SHA-256 digest of everything preceding the KB-JWT
                        ...
                        if (kbPayload.has("exp") && kbPayload.get("exp").asLong() < now) {
                            return new VerificationResult(false, "REJECT_REVOKED_OR_EXPIRED_CREDENTIAL: KB-JWT has expired", header, payload, certRecord, delegatedScopes);
                        }

                        // Cryptographic verification of RFC 9901 KB-JWT signature under Holder key
                        if (kbSections.length == 3) {
                            com.nimbusds.jwt.SignedJWT signedKbJwt = com.nimbusds.jwt.SignedJWT.parse(kbJwtStr);
                            com.nimbusds.jose.jwk.ECKey holderKey = com.nimbusds.jose.jwk.ECKey.parse(certRecord.getRawJwkJson());
                            if (!signedKbJwt.verify(new com.nimbusds.jose.crypto.ECDSAVerifier(holderKey.toECPublicKey()))) {
    ...
    private List<String> extractScopesFromSdJwt(String[] parts, JsonNode payload) throws Exception {
        ...
        for (int i = 1; i < parts.length; i++) {
            ...
            // Verify disclosure digest under SHA-256 against _sd if present
