...
public class TppController {
    ...
    @Value("${BANK_ISSUER_URL:http://tls-tpp-outbound:19443}")
    private String bankIssuerUrl;

    @Value("${DIRECT_BANK_ISSUER_URL:http://bank-issuer:7000}")
    private String directBankIssuerUrl;

    @Value("${RESOURCE_SERVER_URL:http://tls-tpp-outbound:19443}")
    private String resourceServerUrl;

    @Value("${WALLET_URL:http://tls-tpp-outbound:15443}")
    ...
    public String getConfig() {
        ...
        res.put("tppPublicJwk", new JSONObject(cryptoService.getEcJwk().toPublicJWK().toJSONString()));
        res.put("cryptoScheme", "Classical ECDSA P-256");
        return res.toString();
    }

    // ── Dynamic Scope & Identity Policy Discovery ────────────
    ...
    public ResponseEntity<String> getTppAllowedScopes() {
        try {
            ...
            ResponseEntity<String> res = restTemplateConfig.getRestTemplate().getForEntity(policyUrl, String.class);
    ...
    public ResponseEntity<String> requestDelegate(@RequestBody String bodyStr) {
        try {
            ...
            try {
                ...
                ResponseEntity<String> chRes = restTemplateConfig.getRestTemplate().postForEntity(
    ...
    public ResponseEntity<String> approveInWallet(@RequestBody String bodyStr) {
        try {
            ...
            ResponseEntity<String> res = restTemplateConfig.getRestTemplate().postForEntity(
    ...
    public ResponseEntity<String> getPublicDelegateRequest(@PathVariable("requestId") String requestId) {
        ...
        if (curReq != null && requestId.equals(curReq.optString("requestId"))) {
            ...
            fallbackReq.put("tppPublicJwk", new JSONObject(cryptoService.getEcJwk().toPublicJWK().toJSONString()));
    ...
    private ECKey getBankJwksKey() {
        try {
            ...
            ResponseEntity<String> res = restTemplateConfig.getRestTemplate().getForEntity(jwksUrl, String.class);
            ...
            try {
                ...
                System.err.println("[TPP Classical] Failed to fetch Bank JWKS: " + e2.getMessage());
            }
        }
        return null;
    }
    ...
    private boolean verifyAndStoreIncomingVcs(Map<String, JSONObject> tppState, String authzVcStr, String delegateVcStr) {
        ...
        if (authzVcStr != null && !authzVcStr.isEmpty()) {
            try {
                ...
                if (rParts.length < 3) {
                    System.err.println("[TPP Classical] Root Scope VC is malformed");
                    return false;
                }
                ...
                if (rPayload.has("exp") && rPayload.getLong("exp") < now) {
                    System.err.println("[TPP Classical] Root Scope VC has expired");
                    return false;
                }
                rootJti = rPayload.optString("jti", null);

                // Verify Bank Issuer signature (Strict Fail-Closed)
                ECKey bankKey = getBankJwksKey();
                if (bankKey == null) {
                    System.err.println("[TPP Classical] Root Scope VC rejected: unable to fetch Bank public key (fail-closed)");
                    return false;
                }
                boolean bankSigOk = cryptoService.verifySignature(authzVcStr, bankKey);
                if (!bankSigOk) {
                    System.err.println("[TPP Classical] Root Scope VC signature verification failed under Bank key");
                    return false;
                }
            } catch (Exception ex) {
                System.err.println("[TPP Classical] Error verifying Root Scope VC: " + ex.getMessage());
                return false;
            }
        }

        // 2. Verify Delegation VC (F3-04, N-DEL)
        if (delegateVcStr != null && !delegateVcStr.isEmpty()) {
            try {
                String[] dParts = delegateVcStr.split("~")[0].split("\\.");
                if (dParts.length < 3) {
                    System.err.println("[TPP Classical] Delegation VC is malformed");
                    return false;
                }
                ...
                if (dPayload.has("exp") && dPayload.getLong("exp") < now) {
                    System.err.println("[TPP Classical] Delegation VC has expired");
                    return false;
                }
                ...
                if (rootJti != null && !parentJti.isEmpty() && !parentJti.equals(rootJti)) {
                    System.err.println("[TPP Classical] Delegation VC parent_vc_jti ('" + parentJti + "') does not match Root VC JTI ('" + rootJti + "')");
                    return false;
                }
                ...
                if (!sub.isEmpty() && !sub.equals(clientId)) {
                    System.err.println("[TPP Classical] Delegation VC subject ('" + sub + "') does not match TPP client ('" + clientId + "')");
                    return false;
                }

                // Strictly verify wallet signature (F-13)
                if (!dPayload.has("wallet_ec_jwk")) {
                    System.err.println("[TPP Classical] Delegation VC rejected: missing mandatory wallet_ec_jwk");
                    return false;
                }
                ECKey walletKey = ECKey.parse(dPayload.getJSONObject("wallet_ec_jwk").toString());
                boolean sigValid = cryptoService.verifySignature(delegateVcStr, walletKey);
                if (!sigValid) {
                    System.err.println("[TPP Classical] Delegation VC signature verification failed under Wallet key!");
                    return false;
                }
                ...
                if (dPayload.has("cnf") && dPayload.getJSONObject("cnf").has("jwk")) {
                    ...
                    if (!cnfKid.isEmpty() && !myKid.isEmpty() && !cnfKid.equals(myKid)) {
                        System.err.println("[TPP Classical] Delegation VC cnf key mismatch! Expected: " + myKid + ", got: " + cnfKid);
                        return false;
                    }
                }
            } catch (Exception ex) {
                System.err.println("[TPP Classical] Error verifying incoming Delegation VC: " + ex.getMessage());
                return false;
            }
        }
    ...
    public ResponseEntity<String> submitPresentation(@RequestBody String bodyStr) {
        try {
            ...
            logProtocol("Presentation Received (OID4VP)", "incoming",
                "Wallet submitted Classical Verifiable Presentation for request: " + requestId, body);
    ...
    public ResponseEntity<String> pollDelegateStatus(
        ...
        try {
            ...
            try {
                ResponseEntity<String> statusRes = restTemplateConfig.getRestTemplate().getForEntity(
                ...
                if ("APPROVED".equals(status) && !requestId.equals(lastFetched)) {
                    logProtocol("Fetching Approved VCs", "outgoing",
                        "Request approved, fetching Classical VCs from Wallet", null);

                    ResponseEntity<String> vcsRes = restTemplateConfig.getRestTemplate().getForEntity(
                    ...
                    logProtocol("VCs Received from Wallet", "incoming",
                        "Classical Credential payload fetched after user approval", null);
    ...
    public String receiveVcs(@RequestBody String bodyStr) {
        ...
        logProtocol("VCs Received (Push)", "incoming", "Received VCs from Wallet via ECDSA mTLS", null);

        return new JSONObject().put("success", true).toString();
    }

    // ── State / Status ────────────────────────────────────────
    @GetMapping(value = {"/api/state", "/api/status"}, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public ResponseEntity<String> requestToken(@RequestBody(required = false) String bodyStr) {
        try {
            ...
            logProtocol("VP Creation", "internal",
                "Creating Verifiable Presentation JWT with Classical ES256 signature", null);

            String vpJwt = cryptoService.sign(vpPayload.toString(), "JWT");
            ...
            logProtocol("Token Exchange", "outgoing",
                "Sending VP to Bank Issuer via ECDSA mTLS", null);

            ResponseEntity<String> response = restTemplateConfig.getRestTemplate().postForEntity(
    ...
    private String callBankingWithParams(String endpoint, String httpMethod, String requestPayload, String requestId) {
        try {
            ...
            logProtocol("API Call", "outgoing",
                "Calling Resource Server " + endpoint + " via ECDSA mTLS", null);
            ...
            ResponseEntity<String> response = restTemplateConfig.getRestTemplate().exchange(
