...
public class IssuerController {
    ...
    // In-memory stores
    private final Map<String, ObjectNode> issuedVCs = new ConcurrentHashMap<>();
    ...
    private final Map<String, String> keycloakCompositePubKeys = new ConcurrentHashMap<>();
    ...
    private synchronized JWKSet getKeycloakJwks() {
        ...
        String[] endpoints = new String[]{
            ...
            keycloakUrl + "/realms/" + realm + "/hybrid-certs"
        };
        java.util.List<JWK> allKeys = new java.util.ArrayList<>();
        for (String url : endpoints) {
            try {
                JsonNode root = mapper.readTree(new URL(url));
                if (root.has("keys") && root.get("keys").isArray()) {
                    for (JsonNode kn : root.get("keys")) {
                        try {
                            JWK jwk = JWK.parse(kn.toString());
                            allKeys.add(jwk);
                            String kKid = kn.has("kid") ? kn.get("kid").asText() : null;
                            String compPub = kn.has("composite_pub") ? kn.get("composite_pub").asText()
                                    : (kn.has("p384_mldsa65") ? kn.get("p384_mldsa65").asText() : null);
                            if (kKid != null && compPub != null) {
                                keycloakCompositePubKeys.put(kKid, compPub);
                            }
        ...
        if (!allKeys.isEmpty()) {
            cachedKeycloakJwks = new JWKSet(allKeys);
            lastKeycloakJwksFetchTime = now;
            System.out.println("[Bank Issuer] Fetched and cached " + allKeys.size() + " Keycloak JWKS keys across endpoints");
            return cachedKeycloakJwks;
        }
        return cachedKeycloakJwks;
    }
    ...
    private ObjectNode verifyKeycloakAccessToken(String token) {
        try {
            ...
            if (jwkSet == null) {
                System.err.println("[Bank Issuer] Keycloak Hybrid JWKS unavailable to verify access token");
                return null;
            }
            ...
            if (matchedJwk == null) {
                System.err.println("[Bank Issuer] No matching hybrid key found in Keycloak JWKS for kid: " + kid);
                return null;
            }
            ...
            byte[] compositePubBytes = null;
            if (kid != null && keycloakCompositePubKeys.containsKey(kid)) {
                try {
                    String compPubStr = keycloakCompositePubKeys.get(kid);
                    try {
                        compositePubBytes = Base64.getUrlDecoder().decode(compPubStr);
                    } catch (Exception e1) {
                        compositePubBytes = Base64.getDecoder().decode(compPubStr);
                    }
                } catch (Exception ignored) { }
            }

            if (compositePubBytes == null && matchedJwk != null && matchedJwk.toJSONObject() != null) {
                Object compPubObj = matchedJwk.toJSONObject().get("composite_pub");
                if (compPubObj == null) {
                    compPubObj = matchedJwk.toJSONObject().get("p384_mldsa65");
                }
                if (compPubObj instanceof String) {
                    try {
                        compositePubBytes = Base64.getUrlDecoder().decode((String) compPubObj);
                    } catch (Exception e1) {
                        try {
                            compositePubBytes = Base64.getDecoder().decode((String) compPubObj);
                        } catch (Exception ignored) { }
                    }
                }
            }
            ...
            System.out.println("[Bank Issuer Debug] kid: " + kid + ", sigBytes len: " + (sigBytes != null ? sigBytes.length : -1)
                    + ", compositePubBytes len: " + (compositePubBytes != null ? compositePubBytes.length : -1));

            boolean valid = false;
            if (sigBytes != null && sigBytes.length > 256 && compositePubBytes != null && compositePubBytes.length > 0) {
                valid = cryptoService.verifyHybridSignature(signingInput, sigBytes, ecPublicKey, compositePubBytes, "MLDSA65");
            } else {
                try {
                    com.nimbusds.jose.crypto.factories.DefaultJWSVerifierFactory verifierFactory = new com.nimbusds.jose.crypto.factories.DefaultJWSVerifierFactory();
                    java.security.PublicKey pubKey = (matchedJwk instanceof com.nimbusds.jose.jwk.AsymmetricJWK)
                            ? ((com.nimbusds.jose.jwk.AsymmetricJWK) matchedJwk).toPublicKey()
                            : ecPublicKey;
                    com.nimbusds.jose.JWSVerifier verifier = verifierFactory.createJWSVerifier(signedJwt.getHeader(), pubKey);
                    valid = signedJwt.verify(verifier);
                } catch (Exception ex) {
                    System.err.println("[Bank Issuer] DefaultJWSVerifierFactory failed: " + ex.getMessage());
                    valid = cryptoService.verifySignature(signingInput, sigBytes, ecPublicKey);
                }
            }
            if (!valid) {
                System.err.println("[Bank Issuer] Keycloak access token signature verification failed. Alg: " 
                        + signedJwt.getHeader().getAlgorithm() + ", kid: " + kid);
                return null;
            }
    ...
    private String resolveClientCertThumbprint(String clientCertThumbprint, String sslClientCertHeader) {
        ...
        if (sslClientCertHeader != null && !sslClientCertHeader.trim().isEmpty()) {
            try {
                ...
                System.err.println("[PQC IssuerController] Failed to parse ssl-client-cert: " + e.getMessage());
            }
        }
        return null;
    }
    ...
    public ResponseEntity<?> health() {
        ...
        res.put("status", "UP");
        res.put("service", "bank-issuer-java");
    ...
    public ResponseEntity<?> issuerMetadata() {
        ...
        ArrayNode algs = authz.putArray("credential_signing_alg_values_supported");
        algs.add("MLDSA65-ECDSA-P384-SHA512");
        algs.add("ES384");
    ...
    @GetMapping(value = {"/hybrid-certs", "/jwks", "/.well-known/jwks.json"}, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> hybridCerts() throws Exception {
        ObjectNode root = mapper.createObjectNode();
        ArrayNode keys = root.putArray("keys");
        
        // EC JWK (Public Key only)
        ObjectNode jwk = (ObjectNode) mapper.readTree(cryptoService.getEcJwk().toPublicJWK().toJSONString());
        
        // PQC Keys
        ObjectNode pqcKeys = mapper.createObjectNode();
        pqcKeys.put("MLDSA65", cryptoService.getPqcPublicKeyBase64());
        pqcKeys.put("composite_pub", cryptoService.getCompositePublicKeyBase64());
        pqcKeys.put("alg", "MLDSA65-ECDSA-P384-SHA512");
        pqcKeys.put("MLKEM768", cryptoService.getPqcKemPublicKeyBase64());
        jwk.set("pqc_keys", pqcKeys);
    ...
    // TPP Scope Privacy Discovery Endpoint: Returns ONLY the scopes that THIS TPP is permitted to request (S_T)
    // Completely hides User account scope ceiling (S_SC) to preserve Zero Scope Leakage User Privacy.
    ...
    // Public Master Scope Catalog Endpoint
    ...
    // SD-JWT Builder with holder_cert_ref and Constant-size Disclosure Envelopes (padding)
    private ObjectNode buildSdJwtVc(String vct, JsonNode holderJwk, String holderCertRef, ObjectNode claims, List<String> selectiveFields) throws Exception {
        ...
        String bindingProfile = "MLDSA65-ECDSA-P384-SHA512";
        ...
        while (it.hasNext()) {
            ...
            if (selectiveFields.contains(entry.getKey())) {
                ...
                String salt = com.nimbusds.jose.util.Base64URL.encode(saltBytes).toString();
                ...
                String discB64 = com.nimbusds.jose.util.Base64URL.encode(discArr.toString().getBytes(StandardCharsets.UTF_8)).toString();
                disclosures.add(discB64);
                
                MessageDigest md = MessageDigest.getInstance("SHA-384");
                byte[] hash = md.digest(discB64.getBytes(StandardCharsets.UTF_8));
                String hashB64 = com.nimbusds.jose.util.Base64URL.encode(hash).toString();
        ...
        while (disclosures.size() < targetDisclosureCount) {
            ...
            String dummyB64 = com.nimbusds.jose.util.Base64URL.encode(dummyArr.toString().getBytes(StandardCharsets.UTF_8)).toString();
        ...
        String hybridSignedJwt = cryptoService.signHybrid(payload.toString(), "vc+sd-jwt");
        
        StringBuilder sdJwt = new StringBuilder(hybridSignedJwt);
        ...
        result.put("jti", jti);
        result.put("signedComponent", hybridSignedJwt);
    ...
    /**
     * The holder JWK stored in the issuance record and placed in the credential `cnf.jwk` is the classical EC key only.
     * The PQC public key travels as a member of the proof JWK, is read into `pqcPublicKeyBase64` of the record, and
     * must not leak into the committed JWK: the record and the credential are compared as serialized text.
     */
    private static JsonNode withoutPqcMembers(JsonNode jwk) {
        ObjectNode copy = ((ObjectNode) jwk).deepCopy();
        copy.remove("composite_pub");
        copy.remove("pqc_keys");
        return copy;
    }
    ...
    public ResponseEntity<?> credential(
        ...
        try {
            ...
            String pHeader = new String(java.util.Base64.getUrlDecoder().decode(pParts[0]), StandardCharsets.UTF_8);
            JsonNode pHeaderNode = mapper.readTree(pHeader);
            String pqcPubKeyB64 = null;
            JsonNode holderJwk = null;
            if (pHeaderNode.has("jwk")) {
                holderJwk = pHeaderNode.get("jwk");
                if (holderJwk.has("composite_pub")) {
                    pqcPubKeyB64 = holderJwk.get("composite_pub").asText();
                } else if (holderJwk.has("pqc_keys") && holderJwk.get("pqc_keys").has("MLDSA65")) {
                    pqcPubKeyB64 = holderJwk.get("pqc_keys").get("MLDSA65").asText();
                }
                holderJwk = withoutPqcMembers(holderJwk);
            }
            if (holderJwk == null) {
            ...
            byte[] walletPubBytes = (pqcPubKeyB64 != null) ? java.util.Base64.getUrlDecoder().decode(pqcPubKeyB64) : null;
            if (walletPubBytes == null || walletPubBytes.length == 0) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", "invalid_proof", "error_description", "PoP missing mandatory PQC composite public key for PQC baseline"));
            }

            try {
                ECPublicKey proofPubKey = ECKey.parse(holderJwk.toString()).toECPublicKey();
                String pSigningInput = pParts[0] + "." + pParts[1];
                byte[] pSigBytes = java.util.Base64.getUrlDecoder().decode(pParts[2]);
                boolean proofValid = cryptoService.verifyHybridSignature(pSigningInput, pSigBytes, proofPubKey, walletPubBytes, "MLDSA65");
                if (!proofValid) {
            ...
            String pPayload = new String(java.util.Base64.getUrlDecoder().decode(pParts[1]), StandardCharsets.UTF_8);
            ...
            String holderCertRef = certificateRecordStore.registerCertificate(sub, holderJwk, pqcPubKeyB64, clientCertThumbprint);
            ...
            if ("AuthorizationCredential".equals(credType)) {
                ...
                // Canonical Open Banking UK Permissions & Bank Policy Scopes
    ...
    // SD-JWT Parser
    private ObjectNode parseSdJwt(String sdJwt) throws Exception {
        String[] parts = sdJwt.split("~");
        String jwtPart = parts[0];
        String[] jwtSections = jwtPart.split("\\.");
        
        ObjectNode result = mapper.createObjectNode();
        result.put("headerB64", jwtSections[0]);
        result.put("payloadB64", jwtSections[1]);
        result.put("hybridSig", jwtSections[2]);
        
        String headerStr = new String(java.util.Base64.getUrlDecoder().decode(jwtSections[0]), StandardCharsets.UTF_8);
        String payloadStr = new String(java.util.Base64.getUrlDecoder().decode(jwtSections[1]), StandardCharsets.UTF_8);
        result.set("header", mapper.readTree(headerStr));
        result.set("payload", mapper.readTree(payloadStr));
        
        ArrayNode disclosures = mapper.createArrayNode();
        for (int i = 1; i < parts.length; i++) {
            if (!parts[i].isEmpty()) {
                disclosures.add(parts[i]);
            }
        }
        result.set("disclosures", disclosures);
        return result;
    }
    ...
    // F4-F5 Verification Boundary for Token Issuance (Fail-closed pipeline)
    ...
    public ResponseEntity<?> token(
        ...
        try {
            ...
            // Step 1: Authenticate TPP & Validate Authorization VP Assertion
            ...
            JsonNode vpPayload;
            try {
                vpPayloadJson = new String(java.util.Base64.getUrlDecoder().decode(aParts[1]), StandardCharsets.UTF_8);
            ...
            if (!policyService.isTppRegisteredAndActive(tppClientId)) {
                System.err.println("[DAS F4-F5 Boundary] Rejection: TPP client '" + tppClientId + "' is not registered or not active in bank policy");
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "unauthorized_client", "error_description", "TPP client is not active in bank policy"));
            }
            ...
            if (vpPayload.has("ch_das") || vpPayload.has("r_das")) {
                ...
                if (rDas.isEmpty() && vpPayload.has("ch_das")) {
                    ...
                    if (chParts.length >= 2) {
                        JsonNode chPayload = mapper.readTree(new String(java.util.Base64.getUrlDecoder().decode(chParts[1]), StandardCharsets.UTF_8));
            ...
            // Step 2: Independently Verify Root Scope VC (Strict Fail-Closed)
            ...
            if (!rootResult.isValid()) {
                System.err.println("[DAS F4-F5 Boundary] Root Scope VC verification failed: " + rootResult.getErrorMessage());
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "invalid_grant", "error_description", rootResult.getErrorMessage()));
            }
            ...
            // Step 3: Independently Verify Delegation VC (Strict Fail-Closed)
            ...
            if (!delegateResult.isValid()) {
                System.err.println("[DAS F4-F5 Boundary] Delegation VC verification failed: " + delegateResult.getErrorMessage());
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "invalid_grant", "error_description", delegateResult.getErrorMessage()));
            }
            ...
            if (dParts.length > 1 && !dParts[dParts.length - 1].isEmpty()) {
                ...
                if (kbSections.length >= 2) {
                    try {
                        JsonNode kbPayload = mapper.readTree(new String(java.util.Base64.getUrlDecoder().decode(kbSections[1]), StandardCharsets.UTF_8));
                        if (cd != null) {
                        ...
                        System.err.println("[IssuerController] Error parsing KB-JWT in delegation VC: " + kbEx.getMessage());
            ...
            // Step 3b: Verify VP Assertion Signature under TPP's Public Key from Delegation VC
            ...
            try {
                ...
                ECPublicKey tppEcPubKey = null;
                try {
                    tppEcPubKey = ECKey.parse(tppJwkNode.toString()).toECPublicKey();
                } catch (Exception e) {
                    System.err.println("[DAS F4-F5 Boundary] Error parsing TPP ECKey in delegation VC: " + e.getMessage());
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", "invalid_grant", "error_description", "REJECT_INVALID_TPP_JWK: " + e.getMessage()));
                }
                if (!tppJwkNode.has("composite_pub")) {
                    System.err.println("[DAS F4-F5 Boundary] Missing mandatory composite_pub in TPP cnf.jwk for PQC verification");
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", "invalid_grant", "error_description", "REJECT_MISSING_TPP_COMPOSITE_PUB"));
                }
                byte[] tppPubBytes = java.util.Base64.getUrlDecoder().decode(tppJwkNode.get("composite_pub").asText());

                String vpSigningInput = aParts[0] + "." + aParts[1];
                byte[] vpSigBytes = java.util.Base64.getUrlDecoder().decode(aParts[2]);

                boolean vpSigValid = cryptoService.verifyHybridSignature(vpSigningInput, vpSigBytes, tppEcPubKey, tppPubBytes, "MLDSA65");
                if (!vpSigValid) {
            ...
            // Step 4: Compute Effective Scope: S_token = S_SC ∩ S_H ∩ S_R ∩ S_T ∩ S_P
            ...
            // Requested scopes (S_R) from request body or delegate VC
            ...
            if (hasAnyTypedAuthority) {
                if (rootPayload == null || !rootPayload.has("typed_authority")
                    ...
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of(
                            "error", "invalid_grant", "error_description", "REJECT_INCOMPLETE_TYPED_AUTHORITY_SOURCES"
                    ));
                }
                final TypedAuthorityService.Decision typedDecision;
                try {
                    typedDecision = typedAuthorityService.evaluate(
                            rootPayload.get("typed_authority"), holderPayload.get("typed_authority"),
                            vpPayload.get("requested_authority"), vpPayload.get("tpp_authority")
                    );
            ...
            // Step 6: Issue mTLS Sender-Constrained Signed Self-Contained JWT Access Token
            String sub = (delegateResult.getPayload() != null && delegateResult.getPayload().has("sub"))
                    ? delegateResult.getPayload().get("sub").asText()
                    : tppClientId;
            
            // Client certificate thumbprint for mTLS sender constraint (RFC 8705 cnf)
            ...
            cnf.put("x5t#S384", cnfThumbprint);
            ...
            System.out.println("[DAS PQC] Signed JWT Access Token issued for TPP '" + tppClientId + "', exp=300s, scope=" + String.join(" ", s_token));
            return ResponseEntity.ok(res);
    ...
    public ResponseEntity<?> introspect(@RequestParam Map<String, String> body) {
        ...
        if (td != null && td.exp >= System.currentTimeMillis() / 1000) {
            ...
            if (td.cnfThumbprint != null) {
                ObjectNode cnf = mapper.createObjectNode();
                cnf.put("x5t#S384", td.cnfThumbprint);
                res.set("cnf", cnf);
            }
            if (td.vcGrant != null) {
        ...
        // Also check if token is a valid Keycloak access token
        if (token != null && token.contains(".")) {
            ObjectNode kcClaims = verifyKeycloakAccessToken(token);
            if (kcClaims != null) {
                res.put("active", true);
                res.put("sub", kcClaims.has("sub") ? kcClaims.get("sub").asText() : "");
                res.put("scope", kcClaims.has("scope") ? kcClaims.get("scope").asText() : "");
                res.put("exp", kcClaims.has("exp") ? kcClaims.get("exp").asLong() : 0);
                res.put("iss", kcClaims.has("iss") ? kcClaims.get("iss").asText() : "");
                if (kcClaims.has("cnf")) {
                    res.set("cnf", kcClaims.get("cnf"));
                }
                return ResponseEntity.ok(res);
            }
        }
    ...
    @PostMapping(value = "/api/v1/verify-token", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> verifyToken(@RequestBody Map<String, String> body) {
        String token = body.get("token");
        if (token == null || !token.contains(".")) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("valid", false, "error", "Invalid or missing token"));
        }
        String[] parts = token.split("\\.");
        if (parts.length != 3) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("valid", false, "error", "Malformed JWT, expected 3 sections"));
        }
        try {
            String signingInput = parts[0] + "." + parts[1];
            byte[] sigBytes = java.util.Base64.getUrlDecoder().decode(parts[2]);
            byte[] bankCompositePubBytes = java.util.Base64.getUrlDecoder().decode(cryptoService.getCompositePublicKeyBase64());

            // Real Bouncy Castle Native Composite Signature verification
            boolean sigValid = cryptoService.verifyHybridSignature(signingInput, sigBytes, null, bankCompositePubBytes, "MLDSA65");
            if (!sigValid) {
                System.err.println("[IssuerController] Native PQC token verification failed for token");
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("valid", false, "error", "PQC composite signature verification failed"));
            }

            JsonNode payloadNode = mapper.readTree(new String(java.util.Base64.getUrlDecoder().decode(parts[1]), StandardCharsets.UTF_8));
            long now = System.currentTimeMillis() / 1000;
            if (payloadNode.has("exp") && payloadNode.get("exp").asLong() < now) {
                System.err.println("[IssuerController] Native PQC token verification rejected: token has expired");
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("valid", false, "error", "Token has expired"));
            }

            JsonNode headerNode = mapper.readTree(new String(java.util.Base64.getUrlDecoder().decode(parts[0]), StandardCharsets.UTF_8));
            ObjectNode res = mapper.createObjectNode();
            res.put("valid", true);
            res.set("payload", payloadNode);
            res.set("protectedHeader", headerNode);
            return ResponseEntity.ok(res);
        } catch (Exception e) {
            System.err.println("[IssuerController] Error verifying PQC token: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("valid", false, "error", "PQC verification exception: " + e.getMessage()));
        }
    }
